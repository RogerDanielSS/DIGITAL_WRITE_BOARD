...
class DrawingApp(Gtk.Window):
    def __init__(self):
        ...
        self.canvas.add_events(Gdk.EventMask.POINTER_MOTION_MASK | Gdk.EventMask.BUTTON_PRESS_MASK | Gdk.EventMask.BUTTON_RELEASE_MASK)
        self.canvas.connect("motion-notify-event", self.on_mouse_move)
        self.canvas.connect("button-press-event", self.on_mouse_press)
        self.canvas.connect("button-release-event", self.on_mouse_release)
    ...
    def on_mouse_move(self, widget, event):
        if self.drawing:
            ...
            if self.last_mouse_pos is not None:
                cv2.line(self.image, self.last_mouse_pos, self.mouse_pos, (255, 0, 0), 3)  # Cor vermelha (BGR: 0, 0, 255)
    ...
    def update_frame(self):
        ...
        if ret:
            # Não fazemos mais nada com o frame, só o lemos.
            pass
        self.canvas.queue_draw()
        return True
